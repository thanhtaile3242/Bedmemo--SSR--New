        ...
        (() => {
            ...
            let btnDelete_String = '<button type="button" style="position: absolute; right: 15px; top:5px"><i class="fa-solid fa-trash-can"></i></button>';
            const QA = document.getElementsByClassName("form-Q");
            for (let i = 0; i < QA.length; i++) {
                // Create a new button element for each "form-Q" element
                let btnDelete_Node = document.createElement('div');
                btnDelete_Node.innerHTML = btnDelete_String;
                ...
                // Append the delete button to the "form-Q" element
                QA[i].appendChild(btnDelete_Node);
            }
        })();
        // ADD feature
    ...
    `, "text/html");
        ...
        buttonSave.addEventListener('click', function () {
            ...
            inputsArray.forEach(function (item, index) {
                ...
                item.classList.add('q' + classNumber)
            }); // Add class
            ...
            if (checkBlank && checkRadio) {
                for (let i = 0; i < NumberQuestions; i++) {
                    ...
                    // Update id
                let listId = document.getElementsByClassName('id_question')
                for (let i = 0; i < listId.length; i++){
                    folder.questionsArray[i].id = listId[i].value
                }
                // Print the result
                console.log(folder);
